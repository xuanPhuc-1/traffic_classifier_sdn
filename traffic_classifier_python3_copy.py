...
import joblib #to use ML model real-time
from sklearn.decomposition import PCA
proj_location = "/home/xp/trafficclassifier/"
dataset_location = proj_location+"dataset/"
## command to run ##
cmd = "sudo ryu run "+proj_location+"simple_monitor_AK.py"
flows = {} #empty flow dictionary
TIMEOUT = 1*90 #15*60 #15 min #how long to collect training data
...
#function to use the Random Forest model to predict the traffic type and print the table
def printclassifier(model):
    x = PrettyTable()
    x.field_names = ["Flow ID", "Forward Packets", "Forward Bytes", "Forward Delta Packets", "Forward Delta Bytes", "Forward Inst PPS", "Forward Avg PPS", "Forward Inst BPS", "Forward Avg BPS", "Reverse Packets", "Reverse Bytes", "Reverse Delta Packets", "Reverse Delta Bytes", "Reverse Inst PPS", "Reverse Avg PPS", "Reverse Inst BPS", "Reverse Avg BPS", "Traffic Type"]

    #use the Random Forest model to predict the traffic type
    for key,flow in flows.items():
        features = np.array([flow.forward_packets, flow.forward_bytes, flow.forward_delta_packets, flow.forward_delta_bytes, flow.forward_inst_pps, flow.forward_avg_pps, flow.forward_inst_bps, flow.forward_avg_bps, flow.reverse_packets, flow.reverse_bytes, flow.reverse_delta_packets, flow.reverse_delta_bytes, flow.reverse_inst_pps, flow.reverse_avg_pps, flow.reverse_inst_bps, flow.reverse_avg_bps]).reshape(1,-1)
