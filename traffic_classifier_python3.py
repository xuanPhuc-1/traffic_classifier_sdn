...
def printclassifier(model):
    ...
    for key,flow in flows.items():
        #create feature vector for the flow to be classified
        feature_vector = np.array([flow.forward_delta_packets, flow.forward_delta_bytes, flow.forward_inst_pps, flow.forward_avg_pps, flow.forward_inst_bps, flow.forward_avg_bps, flow.reverse_delta_packets, flow.reverse_delta_bytes, flow.reverse_inst_pps, flow.reverse_avg_pps, flow.reverse_inst_bps, flow.reverse_avg_bps])
        feature_vector = feature_vector.reshape(1,-1) #reshape feature vector to be a single sample with multiple features
        label = model.predict(feature_vector)[0] #classify the flow
    ...
    #convert pretty table to dataframe so it can be exported to csv
    df = pd.read_html(x.get_html_string())[0]
